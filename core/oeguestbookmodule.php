...
class oeGuestbookModule extends oxModule
{
    ...
    {
        $sModuleId = 'oeguestbook';

        $this->setModuleData(
            array(
                 'id'          => $sModuleId,
                 'title'       => 'OE Guestbook',
                 'description' => 'OE Guestbook Module',
            )
        );

        $this->load($sModuleId);

        oxRegistry::set('oeGuestbookModule', $this);
    }
    ...
    {
        if ($blUseModulePrefix) {
            $sModuleSettingName = 'oeGuestbook' . (string) $sModuleSettingName;
        }
    ...
    {
        return oxRegistry::getConfig()->getModulesDir() . 'oe/guestbook/';
    }
